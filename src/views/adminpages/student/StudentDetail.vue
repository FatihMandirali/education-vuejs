          ...
          <div class="card-body px-0 pt-0 pb-2">
            <form @submit.prevent="createSubmit">
              <div class="card-body">
                ...
                <select v-model="v$.form.branch.$model" @change="onChange($event.target.value)" style="width: 50%" class="form-control">
                ...
                <select v-model="v$.form.cover.$model" style="width: 50%" class="form-control">
                  <option value="0">{{$t("choose")}}</option>
                  <option v-for="(item,index) in getCoverAllList" v-bind:key="index" :value="item.model.ID">{{item.name}} {{item.surname}}</option>
                </select>
...
export default {
  ...
  async created() {
    await this.setBranchAllListActions().then((response) => {
    ...
    await this.setClassAllListActions().then((response) => {
    ...
    await this.setCoverAllListActions().then((response) => {
      if (response.statusCode === 'ERROR') {
        this.toast.error(this.$t(response.data.message), {
          position: "bottom-right",
          timeout: 3000,
    ...
    await this.setStudentDetailActions(this.$route.params.id).then(res =>{
      ...
      this.form.surname=res.data.data.surname;
      this.form.email=res.data.data.email;
      this.form.phoneNumber=res.data.data.phoneNumber;
      this.form.branch=res.data.data.branchId;
      this.form.class=res.data.data.classId;
      this.form.cover=res.data.data.coverId;
      this.form.isActive=res.data.data.isActive;
      this.onChange(res.data.data.branchId);
      //this.form.cover=res.data.coverId;
  ...
  methods: {
    ...mapActions(["setStudentUpdateActions","setBranchAllListActions","setClassAllListActions","setCoverAllListActions","setStudentDetailActions"]),
    ...mapMutations(["setClassAllListFilterMutations"]),
    createSubmit() {
      console.log(toRaw(this.form))
      this.setStudentUpdateActions(toRaw(this.form)).then(res => {
        if (res.data.statusCode === "ERROR") {
          this.toast.error(this.$t(res.data.message), {
            position: "bottom-right",
            timeout: 3000,
    ...
    onChange(id){
      const request={
        id:parseInt(id),
        allClassList:this.getClassAllList
      }
