    ...
    <div class="shadow-lg card blur">
      <div class="pt-3 pb-0 bg-transparent card-header">
        <div class="float-start">
          <h5 class="mt-3 mb-0">{{$t("studentTable")}}</h5>
          <p>{{$t("filter")}}</p>
        </div>
        <div class="mt-4 float-end" @click="setIsStudentFilterActions(false)">
      ...
      <div>
        ...
        <select v-model="branch" style="width: 100%" class="form-control">
        ...
        <div class="form-check form-check-inline">
          <input class="form-check-input" v-model="isRecord" type="checkbox" id="inlineCheckbox2" value="option2">
          <label class="form-check-label" for="inlineCheckbox2">{{$t("savedStudent")}}</label>
        </div>
        <br>
        <div class="form-check form-check-inline">
...
export default {
  ...
  methods: {
    ...mapActions(["setIsStudentFilterActions","setStudentListActions","setBranchAllListActions"]),
    btnFilter(){
      ...
      this.setStudentListActions(request).then(res =>{
        console.log(res)
      });
    },
    sidenavTypeOnResize() {
